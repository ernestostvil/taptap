...
//// -----------------------------------------Hecho  Ganancia de Ayer---------------------------------------------------------
    $.get("/api/estadistica/gananciaAyer", function (data, status) {
        data.forEach(function (doc) {
            ...
            if(doc[1]===null)
                document.getElementById("gananciaAyer").innerHTML = '$'+0+" GD"
            else
                document.getElementById("gananciaAyer").innerHTML = '$'+doc[1]+" GD"
            console.log(doc)
        });
    });
////-----------------------------------------Hecho Ganancia Utlima Semana---------------------------------------------------------
    $.get("/api/estadistica/gananciaUltimaSemana", function (data, status) {
        data.forEach(function (doc) {
            ...
            if(doc[1]===null){
                document.getElementById("gananciaSemana").innerHTML = '$'+0+" GD"
            }else
                document.getElementById("gananciaSemana").innerHTML = '$'+doc[1]+" GD"
        });
    });
